...
- clarify: the question has several reasonable readings that give different answers ("what's selling well?": by revenue? by units?). Give 2-4 concrete options, each answerable as a plan.
...
{"type":"case","when":[{"if":PRED,"then":VALUE}…],"else":EXPR}   (else is required; VALUE is a col, lit or param)
...
PRED:
{"op":"eq|ne|lt|lte|gt|gte","left":VALUE,"right":VALUE}   (VALUE = {"type":"col"…} | {"type":"lit"…} | {"type":"param"…})
